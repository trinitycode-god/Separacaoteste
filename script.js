...
$("#btn-baixar-separacao").addEventListener("click", async () => {
  ...
  try{
    await gerarEtiquetaPDF(state.currentOP, state.itens);
  } catch(err){
...
async function gerarEtiquetaPDF(op, itens){
  ...
  const linhas = itens.map((it, i) => [
    ...
    it.quantidade_separada || it.quantidade || "—",
    it.usuario || "—",
  ...
  doc.autoTable({
    ...
    head: [["ITEM", "MICROSIGA", "DESCRIÇÃO", "ENDEREÇO", "LOTE", "QTD", "SEPARADO POR", "DATA/HORA DA SEPARAÇÃO"]],
    body: linhas,
    styles: { font: "helvetica", fontSize: 8, cellPadding: 5, lineColor: [0, 0, 0], lineWidth: 0.6, valign: "middle" },
    headStyles: { fillColor: [10, 10, 10], textColor: 255, fontStyle: "bold", halign: "center" },
