...
// =====================================================================
const CONFIG = {
  APPS_SCRIPT_URL: "https://script.google.com/macros/s/AKfycbxePfWul-Pt-GxL8COuUOl115blh1ziOXCSnNlc2deDcMRZaiSk814767D2NjmOKESo/exec"
};
...
async function api(action, payload){
  try{
    ...
    mostrarVersaoBackend(data.version);
    if (!data.ok){
...
function mostrarVersaoBackend(versao){
  let selo = document.getElementById("selo-versao-backend");
  if (!selo){
    selo = document.createElement("div");
    selo.id = "selo-versao-backend";
    selo.style.cssText = "position:fixed; bottom:8px; right:8px; z-index:99999; background:#111; color:#fff; font-family:monospace; font-size:11px; padding:6px 10px; border-radius:6px; border:1px solid #444; opacity:.9;";
    document.body.appendChild(selo);
  }
  selo.textContent = versao ? ("Backend: " + versao) : "Backend: SEM CAMPO VERSION (código antigo)";
  selo.style.background = versao ? "#0f5c2e" : "#7a1f1f";
}
